package hcxm.air.sql {
	...
	[Event(name="onCreateDataBase", type="hcxm.air.sql.events.ConnectEvent")]
	[Event(name="onCreateTable", type="hcxm.air.sql.events.ConnectEvent")]
	[Event(name="onInsertDataComplete", type="hcxm.air.sql.events.ConnectEvent")]
	[Event(name="onSelectComplete", type="hcxm.air.sql.events.ConnectEvent")]
	[Event(name="onSchema", type="hcxm.air.sql.events.ConnectEvent")]
	[Event(name="onError", type="hcxm.air.sql.events.ConnectEvent")]
	[Event(name="onSelectCountComplete", type="hcxm.air.sql.events.ConnectEvent")]
	[Event(name="onSelectExecComplete", type="hcxm.air.sql.events.ConnectEvent")]
	...
	public class Connect extends EventDispatcher implements IEventDispatcher {
		 ...
		 */
		public function Connect(sqlConn,_db,isencry:Boolean=false,encryptionKey=null) {
		 ...
		 * @param	str	表的字段定义
		 */
		public function createTable(name:String, str:String) {
			...
			var sql:String =  "CREATE TABLE IF NOT EXISTS " + _tableName + " ("+str+ ")";
			createStm.text = sql;
		 ...
		 * @param	ar2	对应字段的值
		 */
		public function insertData(ar1:Array,ar2:Array) {
			var createStm:SQLStatement = new SQLStatement();
			createStm.sqlConnection = sqlConn;
			...
			var sql:String = "INSERT INTO " + tableName + " (" + s1 +") VALUES (" + s2 + ")";
			trace( "sql : " + sql );
		...
		public function updateData(ar1:Array,ar2:Array,str:String) {
			...
			var sql:String = "UPDATE " + tableName+" SET" + s+" where "+str;
			trace( "sql : " + sql );
		//	var sql:String = "INSERT INTO employees (firstName, lastName, salary) " + "VALUES ('Bob', 'Smith', 8000)";
			createStm.text = sql;
			createStm.execute();
		...
		private function onSqlSchema(e):void {
			for( var i:String in e ) trace( "key : " + i + ", value : " + e[ i ] );
		 ...
		 * 查询所有数据
		 */
		public function selectAll(str:String=null) {
		 ...
		 * 查询表中的记录总数
		 * */
		public function selectCount(str:String=null) {
		...
		public function selectLike(str:String):void
		{
			...
			var sql:String="select * from "+this.tableName+" where message like :str"
		 ...
		 * */
		 ...
		 {
		 	var sql:String="select * from "+tableName+" where strftime('%Y-%m-%d',"+filed+")='"+FormatDate.toDate(data)+"'";
			var sqlsment:SQLStatement = new SQLStatement();
			sqlsment.sqlConnection = sqlConn;
			sqlsment.text =sql;
			sqlsment.execute();
		...
		private function _onError1(e):void
		{
